...
export default function AddVideoPage() {
    ...
    try {
      const startResponse = await axios.post(
        API_URL + '/camera/cameras/scan-video/start',
        {
          ...
          min_weapon_images: 3,
          max_weapon_images: 5,
          capture_name: selectedFile?.name ? selectedFile.name.replace(/\.[^/.]+$/, '') : 'video_scan',
        },
        {
          headers: {
      ...
      scanPollRef.current = setInterval(async () => {
        try {
          const statusResponse = await axios.get(API_URL + '/camera/cameras/scan-video/status/' + jobId)
  ...
  const resultImages = Array.isArray(scanResult?.weapon_images) ? scanResult.weapon_images : []

  return (
    <div className='px-4 md:px-0'>
      <div className='container px-2 md:px-8 mt-6 md:mt-8'>
        <h1 className='text-3xl py-2 font-semibold'>{t('addVideoTitle')}</h1>
        <p className='text-black-600 mb-6 text-sm md:text-base'>{t('selectCameraForVideo')}</p>

        <div className='mb-6 bg-white rounded-md p-4 md:p-6 shadow-sm ring-1 ring-inset ring-gray-200'>
          <label htmlFor='video_file_global' className='block text-sm font-semibold leading-6 text-black-600'>
            {t('selectVideoFile')}
          </label>
          <div className='mt-2.5 flex flex-col md:flex-row gap-3'>
            ...
            >
              {isScanning ? t('processingVideoPleaseWait') : t('scanUploadedVideo')}
            </button>
          </div>
          <div className='mt-3 grid grid-cols-1 md:grid-cols-2 gap-3'>
            ...
            <div>
              <label htmlFor='compute_device' className='block text-xs font-semibold text-black-600 mb-1'>
                {t('computeDevice')}
              </label>
              <select
                id='compute_device'
            ...
            <div>
              <label htmlFor='scan_profile' className='block text-xs font-semibold text-black-600 mb-1'>
                {t('scanProfile')}
              </label>
              <select
                id='scan_profile'
                ...
                className='block w-full rounded-md border-0 px-3.5 py-2.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300'
              >
                <option value='fast'>{t('scanProfileFast')}</option>
                <option value='balanced'>{t('scanProfileBalanced')}</option>
                <option value='accurate'>{t('scanProfileAccurate')}</option>
              </select>
            </div>
          </div>
          <div className='mt-3 grid grid-cols-1 md:grid-cols-1 gap-3'>
            <div>
              <label htmlFor='snapshot_interval' className='block text-xs font-semibold text-black-600 mb-1'>
                {t('weaponSnapshotInterval')}
              </label>
              <select
                id='snapshot_interval'
                value={snapshotIntervalSeconds}
                onChange={(e) => setSnapshotIntervalSeconds(Number(e.target.value))}
                className='block w-full rounded-md border-0 px-3.5 py-2.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300'
              >
              ...
              {isScanning ? (
                <div className='mt-2'>
                  ...
                  <span className='block mt-1 text-xs'>
                    {t('scanProgress')}: {scanProgress}% - {t('remainingTime')}: {formatEta(scanEtaSeconds)}
                  </span>
                </div>
              ) : null}
              {scanResult && scanResult.weapon_found ? (
                <div className='mt-2 flex flex-wrap items-center gap-2'>
                  <span className='text-xs'>
                    {t('detectedImagesCount')}: {scanResult.weapon_images_count || 0}
                  </span>
                  <button
                    type='button'
        ...
        <ul className='grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4'>
          {cameras.map((cam) => (
            <li key={cam.id} className='bg-white drop-shadow-md p-4 rounded-md ring-1 ring-inset ring-gray-200'>
              <div className='font-semibold text-lg'>{cam.name}</div>
              <div className='text-sm text-black-600'>{cam.location || '-'}</div>
          ...
          <div className='w-full max-w-5xl rounded-lg bg-white shadow-xl ring-1 ring-gray-200'>
            ...
            <div className='max-h-[70vh] overflow-auto p-4'>
              {resultImages.length === 0 ? (
                <p className='text-sm text-black-600'>{t('noWeaponDetected')}</p>
              ) : (
