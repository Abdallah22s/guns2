  ...
  return (
    <div className="group bg-white rounded-2xl shadow-sm hover:shadow-xl border border-gray-100 overflow-hidden transition-all duration-300 hover:-translate-y-1">
      <div className="relative">
        ...
        <Link href={routerBase + "dashboard/cameras/" + camera.id}>
          <div className="aspect-video bg-gradient-to-br from-gray-100 to-gray-200 relative overflow-hidden">
            <img
              ...
              src={routerBase + "images/cam1.svg"}
              alt={camera.name}
            />
        ...
        <div
          className={`absolute top-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform ${isRTL ? "-translate-x-2 group-hover:translate-x-0 left-3" : "translate-x-2 group-hover:translate-x-0 right-3"}`}
        >
          <Link
      ...
      <div className="p-4">
        ...
        <div className="mt-4 pt-4 border-t border-gray-100 flex gap-2">
          ...
          <Link
            href={routerBase + "dashboard/cameras/test/" + camera.id}
            className="flex items-center justify-center gap-2 px-3 py-2 bg-green-50 hover:bg-green-100 text-green-700 hover:text-green-800 rounded-lg text-sm font-medium transition-colors"
            title={t("test") || "Test"}
          >
            <svg
              className="w-4 h-4"
...
export default function AllCamerasPage() {
  ...
  const handleSearch = async (query) => {
    ...
    if (!query.trim()) {
      ...
      try {
        const response = await axios.get(API_URL + "/camera/cameras");
        const updateData = { ...state, cameras: response.data };
        setState(updateData);
      } catch (error) {
        console.error("Error fetching cameras:", error);
    ...
    try {
      const response = await axios.get(
        API_URL + "/camera/cameras/search?q=" + encodeURIComponent(query),
      );
      if (response.data && response.data.results) {
        const updateData = { ...state, cameras: response.data.results };
        setState(updateData);
      }
    } catch (error) {
  ...
  return (
    <div
      className={`min-h-screen bg-gray-50 py-8 ${isRTL ? "rtl" : "ltr"}`}
      dir={isRTL ? "rtl" : "ltr"}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-6">
          <AdvancedFilterPanel
            ...
            searchPlaceholder={t("searchCameras")}
          />
        </div>
        ...
        {!isLoading && (!state.cameras || state.cameras.length === 0) && (
          <div className="text-center py-20 bg-white rounded-2xl border border-gray-200">
            ...
            <h3 className="text-xl font-semibold text-gray-900 mb-2">
              {searchQuery ? t("noCamerasForSearch") : t("noCamerasYet")}
            </h3>
            <Link
              href={routerBase + "dashboard/cameras/add"}
              className="inline-flex items-center gap-2 mt-4 text-blue-600 hover:text-blue-700 font-medium"
            >
              {searchQuery ? t("clearSearch") : t("addFirstCamera")}
              <svg
                className="w-4 h-4"
                fill="none"
                stroke="currentColor"
        ...
        {!isLoading && state.cameras && state.cameras.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {state.cameras.map((camera) => (
              <CameraCard key={camera.id} camera={camera} isRTL={isRTL} t={t} />
            ))}
          </div>
        )}
      </div>
