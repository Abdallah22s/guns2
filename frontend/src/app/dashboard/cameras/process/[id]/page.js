        ...
        try {
          ...
          if (
            ...
            if (jobStatus.status === "failed") {
              ...
              if (
                jobStatus.weapon_images &&
                jobStatus.weapon_images.length > 0
              ) {
                const verifiedWeaponImages = jobStatus.weapon_images.filter(
                  (imageName) =>
                    typeof imageName === "string" &&
                    imageName.startsWith("weapon_"),
                );
                if (verifiedWeaponImages.length > 0) {
                  const updatedWeapons = verifiedWeaponImages.map(
                    (imageName, index) => ({
                      ...
                      confidence: Math.round(
                        parseFloat(
                          imageName.split("_")[2].replace(".jpg", ""),
                        ) * 100,
                      ),
                    }),
                  );
                  ...
                  window.dispatchEvent(
                    new CustomEvent("weaponAlert", { detail: updatedWeapons }),
                  );
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
          ...
          <div className="p-6 lg:p-8 space-y-8">
            ...
            {/* File Upload */}
            <FileUpload onFileSelect={handleFileSelect} isRTL={isRTL} />
      ...
      <WeaponResultsModal
        ...
        onDeleteImage={async (weapon) => {
          try {
            ...
            setWeapons((prev) =>
              prev.filter((w) => w.image_path !== weapon.image_path),
            );
          } catch (err) {
