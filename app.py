...
def serve_file(filename):
    return send_from_directory(str(INFERENCES_DIR), filename)


def start_wep_instance(ini_file):
    # on Windows the shell script won't run; skip to avoid errors
    if sys.platform.startswith("win"):
        print("Windows detected - skipping wep instance for", ini_file)
        return
    try:
        result = subprocess.run(
            ["./weprunner.sh", ini_file],
            check=True,
            shell=False,
            capture_output=True,
            text=True,
...
def start_webhook():
    if sys.platform.startswith("win"):
        print("Windows detected - skipping webhook startup")
        return
    try:
        result = subprocess.run(
            ["./web_hook.sh"], shell=False, capture_output=True, text=True
        )
    except Exception as e:
...
if __name__ == "__main__":
    ...
    for camera in cameras.get_cameras():
        ...
        wep_process = mp.Process(
            target=start_wep_instance, args=(cameras.to_camera_ini_name(camera),)
        )
