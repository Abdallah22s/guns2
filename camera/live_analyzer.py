    ...
    def _analyze_stream(self):
        ...
        try:
            while self.is_running:
                ...
                if not ret:
                    # Try to reconnect
                    logger.warning(f"Lost connection to stream, attempting reconnect...")
                    time.sleep(2)
                    cap = cv2.VideoCapture(self.stream_url)
                    if not cap.isOpened():
                    ...
                    if scores is not None and len(scores) > 0 and scores.shape[1] > 0:
                        ...
                        if confidence > self.confidence_threshold:
                            ...
                            detection = {
                                ...
                                "image_path": str(filepath.relative_to(WEPAPP_INFERENCES_DIR)),
                            }
                            
                            self.detections.append(detection)
                            detected_count += 1
                            
                            logger.info(f"⚠️ weapon detected! Camera: {self.camera_name}, Confidence: {confidence:.2%}")
...
def stop_live_analysis(camera_id: int) -> dict:
    ...
    return {
        "success": False,
        "message": "No active analysis for this camera"
    }
