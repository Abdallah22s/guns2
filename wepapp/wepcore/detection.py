...
def detect(msg_py, msg_local_py, msg_passing):
    ...
    if msg_local_py is None:
        ...
        if ((video_ext == "rtsp") or (video_ext == "mjpeg")) and (
            (video_link == "") or (video_friendly_name == "")
        ):  # validation
            now = datetime.now()
            dt_string = now.strftime("%d-%m-%Y %H:%M:%S")
            data1 = {
                "status": "Video link or friendly name is blank.",
            ...
            msg_py["payload"] = (
                frame_sum_arr_except  # this will be displayed in fronted using socket
            )
        ...
        # when video url or friendly name is empty should add that to exception
        if ((video_ext == "mp4") or (video_ext == "webm")) and (
            video_friendly_name == ""
        ):  # validation
            ...
            frame_sum_arr_except.append(data1)
            msg_py["payload"] = (
                frame_sum_arr_except  # this will be displayed in fronted
            )
            msg_passing.append(msg_py)
        ...
        if video_ext in ["mp4", "webm"]:
            ...
            try:
                ...
                vid = cv2.VideoCapture(
                    str(video_path)
                )  # to get height, width of the frame to save
                vid_latest_rtsp = cv2.VideoCapture(
                    video_path
                )  # Read the latest frame from rtsp stream
        ...
        if os.path.exists(cfg.output_path + video_name):
            shutil.rmtree(
                cfg.output_path + video_name
            )  # remove directory even it contains files inside
        ...
        try:
            ...
            while keep_trying == True:
                ...
                weapon_arr = (
                    []
                )  # here after each loop weapon_arr should be made empty to reduce the memory consumption
                ...
                if video_ext in ["mp4", "webm"]:
                    ...
                    if return_value:
                        ...
                        if (
                            frame_num != 0
                        ):  # if video format issue frame_num will be 0. if >0 then it is coming after inference is completed
                            ...
                            msg_py["payload"] = (
                                arr_comp  # this will be displayed in fronted using socket
                            )
                            msg_passing.append(msg_py)
                            ...
                            log.error(
                                "Exception in the current frame: {}".format(data1)
                            )
                            frame_sum_arr_except.append(data1)
                            msg_py["payload"] = (
                                frame_sum_arr_except  # this will be displayed in fronted using socket
                    ...
                    try:
                        ...
                        if not return_val:
                            ...
                            if cnt > cfg.bad_frame_skip_size:
                                ...
                                cnt = 0
                                log.info(
                                    "within cnt > {}".format(cfg.bad_frame_skip_size)
                                )
                        ...
                        if frame_num % 15 == 0:
                            cv2.imwrite(
                                cfg.output_path + video_name + "/" + "live.jpg", frame
                            )
                        if frame is None:
                            continue
                        frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                ...
                # start = time.time()
                image, start_time2, end_time2, scores_weapon, classes_weapon = (
                    inference_images_weapon(frame, video_name, frame_num)
                )  # inference
                # time_taken = (time.time() - start )
                fps = 1.0 / (time.time() - start_time2)
                log.info("FPS01: %.2f" % fps)
                # In YOLOv8-only mode, scores/classes are already numpy arrays.
                ...
                for s, c in zip(scores_weapon[0], classes_weapon[0]):
                    ...
                    if float(s) > 0:
                        if int(c) == 0:
                            detection_class = "Gun : " + str(
                                s
                            )  # Added score for prediction also
                        elif int(c) == 1:
                            ...
                            if float(s) > knife_threshold:
                                detection_class = "Knife : " + str(
                                    s
                                )  # Added score for prediction also
                            else:
                                continue
                        elif int(c) == 2:
                            detection_class = "Rifle : " + str(
                                s
                            )  # Added score for prediction also
                        else:
                            detection_class = "None"
                        weapon_threats_in_frame.append(detection_class)
                        threat_status = "Threat detected | Weapon:" + str(
                            weapon_threats_in_frame
                        )
                        ...
                        if video_ext in ["mp4", "webm"]:
                            ...
                            image_capture_path = (
                                "weapon_" + str(round(time_taking, 2)) + ".jpg"
                            )
                            now = datetime.now()
                            dt_string = now.strftime("%d-%m-%Y %H:%M:%S")
                            cv2.imwrite(
                                cfg.output_path + video_name + "/" + image_capture_path,
                                image1_write,
                            )
                            weapon_arr.append(image_capture_path)
                            data_weapon = {
                                "status": detection_type_weapon,
                                "video_name": video_name1,
                                "datetime": dt_string,
                                "time(s)": str(round(time_taking, 2)),
                                "Threat_status": threat_status,
                                "image_path": image_capture_path,
                                "weapon_images": weapon_arr,
                            }
                        elif video_ext in ["rtsp", "mjpeg"]:
                            now = datetime.now()
                            dt_string = now.strftime("%d-%m-%Y %H:%M:%S")
                ...
                if (detection_type_mask == "no_mask") and (
                    detection_type_weapon == "no_weapon"
                ):
                ...
                if threat_status != "Threat not detected":
                    ...
                    if (
                        len(frame_sum_arr) <= 10
                    ):  # only update last update key for 1st 10 frames
                        if len(frame_sum_arr) != 0:
                            frame_sum_arr_new = (
                                []
                            )  # change old frames last_updated status false and only keep for last frame as True
                            for dic in frame_sum_arr[0 : len(frame_sum_arr) - 1]:
                                data_old = {
                                    "status": dic["status"],
                                    "video_name": dic["video_name"],
                                    "video_type": dic["video_type"],
                                    "datetime": dic["datetime"],
                                    "Threat_status": dic["Threat_status"],
                                    "last_updated": False,
                                }
                        ...
                        log.warn(
                            "{} Detected in Frame: {}".format(
                                detection_class, str(frame_num)
                            )
                        )
                    elif (
                        len(frame_sum_arr) == 11
                    ):  # if lenth is 11 remove 1st element keep only last 10 elements. Also update last_update key
                        frame_sum_arr_new = []
                    ...
                    if video_ext in ["mp4", "webm"]:
                        output_format = "vp80"
                        log.info(
                            "output assignment. format{} path:{}".format(
                                output_format, cfg.output_path
                            )
                        )
                        ...
                        out = cv2.VideoWriter(
                            output_path, codec, fps / 5, (width, height)
                        )

                msg_py["payload"] = (
                    frame_sum_arr  # this will be displayed in fronted using socket
                )
                msg_passing.append(msg_py)
                msg_py["payload"] = (
                    data2  # send frame to show video in real time in frontend
                )
                msg_passing.append(msg_py)
